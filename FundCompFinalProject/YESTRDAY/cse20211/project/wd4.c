...
int main()
{
    ...
    health = &hp;
    int scores[11] = {0}, score = 0, diff = 1, typeLoc = 0, typeCheck = 0; //TODO: change back to 100
    ...
    gfx_text(centX-140, centY - 10, "Type the words on the screen to keep them from falling.");
    gfx_text(centX-160, centY + 20, "Text field clears when pressing any non-alphabetic character");
   ...
   //             printf("%c is not alph\n", c); //TODO: DELETE TEST LINE
       ...
       //     printf("typeLoc: %d input: %s\n", typeLoc, input); //TODO: DELETE TEST LINE
        ...
        if(strcmp(input, "") == 0)         gfx_text(centX - 150, 550, "[ENTER TEXT TO SAVE THE WORLD]");
        else                                gfx_text(centX - 100, 550, input);
        /*----------*/
    ...
    gfx_text(centX-60, centY-90, "GAME OVER");
...
void Update(Word_type *p, int *health, int diff, char words[100][25])
{
    ...
    if (p->vectorY >= 500)
    {
        ...
        p->xDelta = (rand() % 2) * rand() % (2 + diff) / diff; 

        p->yDelta = 0;
        
        p->yDeltaT = 1 + rand() % (3 * diff) / diff;
