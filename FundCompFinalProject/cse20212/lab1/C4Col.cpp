...
char C4Col::getDisc(int row){           //member function getDisc(int row) returns the desired element from the private character array Discs established in C4Col.h
    return Discs[row];
}


int C4Col::getMaxDiscs(){               //member function getMaxDiscs() returns the maximum number of discs
    return maxDiscs;
}
