...
void loadFiles(void)
{
	FILE *fstat;
	int n=3;	lesson l;
	if((fulist = fopen("userlist.dat","rb+")) == NULL)	/*if file is not
created or is missing*/
	{
		printf("\n\nFile does not exist..\n Creating new...");
...
int mainmenu(void)
{
	...
	cprintf("\n\n\r \xaf Press the arrow keys: \x18 or \x19 to navigate
the options.\
...
{/* here the n user name list is displayed from the file ulist.dat
	...
	if( (fuser = fopen(userfilename,"r+b")) == NULL )	/*opened for the
beginSession() to write session details*/
	{
		printf("\n\nInvalid filename...");
		printf("\npress any key to continue...");
		getch();
...
void beginSession()
{
	...
	while(i<txtlen)
  {
     ...
     window(1,1,80,3);
	   ...
	   textcolor(MAGENTA);
     clrscr();
	   cprintf("page %d of %d",currentpage,npage);	/*display also title
username...*/
     cprintf("\n\rCurrent User : %10s",cu.name);
     cprintf("\n\rCurrent Lesson : %10s",cl.title);
     box(x2,y2,PARAWIDTH,NLINES,11,BLACK);
  ...
  cprintf("\n\r%s,%s %2d,
%d",day[cs.dtntm.tm_wday],month[cs.dtntm.tm_mon],cs.dtntm.tm_mday,cs.dtntm.tm_year+1900);
  cprintf("\n\rstart time [hh:mm:ss] =
[%02d:%02d:%02d]",cs.dtntm.tm_hour,cs.dtntm.tm_min,cs.dtntm.tm_sec);
...
void viewStat(void)
{
  ...
  n=ftell(fstat)/sizeof(session);
	...
	printf("\n\nsn
%10s%10s%7s%10s%8s%12s%10s","Name","Lesson","WPM","Accuracy","Score","Date","Time");
...
void sortSession(session list[],int n)
{
	...
	for(p=1;p<n;p++)
	{
		tmp=list[p];
		for(j=p;j>0&&(
(list[j-1].accuracy*(float)(list[j-1].wpm))<(tmp.accuracy*(float)(tmp.wpm))
);j--)
			list[j] = list[j-1];
		list[j] = tmp;
	}
...
void viewRecords(void)
{
	...
	printf("\n\nsn
%10s%7s%10s%8s%12s%10s","Lesson","WPM","Accuracy","Score","Date","Time");
